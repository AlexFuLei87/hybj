...
document.onkeydown = function(){
  ...
  <body>
    <%-- <table width="100%" border="0" id="table8">
	    <tr>
		<td style="width: 30%;">节目名字</td>
		<td style="width: 30%;">所属栏目</td>
		<td style="width: 20%;">类型</td>
		<td style="width: 25%;">备注</td>
		</tr>
	<s:if test="#request.cpList!=null">
	<s:iterator value="%{#request.cpList}" var="list">
		<tr>
		<td style="width: 30%;">${list.itemName }</td>
		<td style="width: 30%;">${list.programaName }</td>
		<td style="width: 20%;">${list.type }</td>
		<td style="width: 25%;">${list.remarks }</td>
		</tr>
	</s:iterator>	
 	</s:if>
	</table> --%>
		...
		<form id="myform" name="form" action="../combine/selectRight.do" method="post">
