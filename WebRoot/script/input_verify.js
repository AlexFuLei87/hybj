 ...
 * @returns {Boolean}
 */
function verifySalary(str, varSalary, maxLen, isExplicit) {
    if (isEmpty(str)) {
        if (isExplicit) {
            alert("输入字符串(" + varSalary + ")不能为空，请重新输入！");
        }
        return false;
    }
    ...
    if ((str + "").length > maxLen) {
        if (isExplicit) {
            alert("输入字符串(" + varSalary + ")长度不能超过" + maxLen + "个字符，请重新输入！");
        }
        return false;
    }
    var reg = /(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/;
    if (!reg.test(str)){
        alert(varSalary + "的输入格式不正确,请确认!");
        return false;
    }
    return true;
}